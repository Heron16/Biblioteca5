...
class LibrosModel extends Query
{
    ...
    public function verificarReserva($id_libro)
    {
        $sql = "SELECT COUNT(*) as total FROM reservas WHERE id_libro = $id_libro AND estado = 1";
        $res = $this->select($sql);
        return $res['total'] > 0;
    }
    
    public function getCantLibro($id)
    {
        $sql = "SELECT cantidad FROM libro WHERE id = $id";
        $res = $this->select($sql);
        return $res;
    }
}
