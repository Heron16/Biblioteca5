...
class Libros extends Controller
{
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $reservado = $this->model->verificarReserva($data[$i]['id']);
            ...
            if (isset($_SESSION['usuario']) && $_SESSION['usuario'] == 'angel') {
                ...
                if ($data[$i]['estado'] == 1) {
                    if ($reservado) {
                        $data[$i]['estado'] = '<span class="badge badge-warning">Indisponível</span>';
                        $data[$i]['acciones'] = '<div>
                        <span class="text-muted">Livro indisponível</span>
                        </div>';
                    } else {
                        $data[$i]['estado'] = '<span class="badge badge-success">Disponível</span>';
                        $data[$i]['acciones'] = '<div>
                        <span class="text-success">Livro disponível</span>
                        </div>';
                    }
                } else {
                    $data[$i]['estado'] = '<span class="badge badge-danger">Indisponível</span>';
                    $data[$i]['acciones'] = '<div>
                    <span class="text-muted">Livro indisponível</span>
                    </div>';
                }
                ...
                if ($data[$i]['estado'] == 1) {
                    if ($reservado) {
                        $data[$i]['estado'] = '<span class="badge badge-warning">Reservado</span>';
                    } else {
